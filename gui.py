from pygame import *
from geometry import inRect
init()
screen = display.set_mode()

class Rect:
    def __init__(self, rect, colour, width=0):
        self.rect = rect
        self.colour = colour
        self.width = width
    def draw(self):
...
class Button:
    def __init__(self, rect, function, colour=(150,150,150), hoveredColour=(200,200,200), sprite=None):
        self.rect = rect
    ...
    def getHovered(self, mousePos, click=False): # can also be used for clicks
        if inRect(mousePos, self.rect):
            self.hovered = True
            if click:
                self.function()
        else:
    ...
    def draw(self):
        if self.hovered:
            draw.rect(screen, self.hoveredColour, self.rect)
        if self.sprite == None:
